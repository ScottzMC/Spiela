...
.contacts-emails label {text-transform:none;}
.txt-note {font-size: 12px;text-transform:none;}
.btn-invite {width: 40px;border-radius: 50%; float: left;}
.contacts-emails .form-row{margin-bottom: 6px;}
.invite-send {width: 40px;border-radius: 50%;cursor: none !important;}
.snackbar {
  visibility: hidden;
  min-width: 100px;
  /*margin-left: -125px;*/
...
.loader-img.show {display: block !important;}
.contacts-emails {word-wrap: anywhere;}
.btn-blue {background-color:#00B9F6;}

@media only screen and (max-width: 600px) {
  hr{display: block !important;border-top: 1px solid #00B9F6;}
}
</style>
			...
			<div class="spiela-container">
                <div class="login-wrap centered">
                        <div id="normal-header">
                            <h3 class="text-center font-weight-bold">Invite Users</h3>
                            <h5 class="text-center">Join the community in a matter of seconds!</h5>
                        </div>
            
                    <div class="d-flex justify-content-between flex-column login-snippet-wrap">
                        <div>
                            <form action="<?php echo base_url('invite'); ?>" method="POST">
                                <div class="form-group">
                                    <label for="email">Email addresses</label>
                                    <input type="email" class="form-control" id="email" name="email" multiple placeholder="JohnDoe@Spiela.co.uk">
                                    <label for="email" class="txt-note">Note: Multiple emails with "," are allowed.</label>
                                </div>                            
                                <button class="btn post-post-button" type="submit" name="invite"> Submit</button>
                            </form>
                            ...
                            <div class="col-md-6 form-input-wrapper cst mt-4 text-left position-relative" style="width: 250px; margin-left: -30px;">
                                <a href="https://accounts.google.com/o/oauth2/auth?client_id=<?php print $google_client_id; ?>&redirect_uri=<?php print $redirect_uri; ?>&scope=https://www.google.com/m8/feeds/&response_type=code" class="btn btn-primary btn-block" style="background: #DD4B39;border-color: #DD4B39"><i class="fab fa-google"></i> Invite via Google!</a>
                            </div>                            
                            <?php else : ?>
                                <?php //echo '<pre>'; print_r($contacts);die; ?>
                                <?php if(isset($contacts) && count($contacts) > 0) : ?>    
                                    <div class="contacts-emails">    
                                        <h4 class="text-left font-weight-bold">Contacts List :</h3>
                                        <?php foreach($contacts as $key => $contact) : ?>

                                            <?php $displayName = $contact['name']; 
                                            if($contact['name'] == ''){
                                                $displayName = $contact['email']; 
                                            }
                                            ?>
                                            <!-- <input type="checkbox" id="contact<?php //echo $key; ?>" name="contact<?php //echo $key; ?>" class="contactscheckbox" value="<?php //echo $contact['email']; ?>"> -->
                                            <div class="form-row align-items-center">
                                                <div class="col-sm-8">
                                                    <label for="contact<?php echo $key; ?>"><?php echo $displayName; ?></label>
                                                </div> 
                                                <div class="col-sm-4" style="float: left;">
                                                    <div class="invite-<?php echo $key; ?>" style="float: left;">
                                                        <button class="btn post-post-button btn-invite " type="button" onclick="sendInvitation('<?php echo $contact['email']; ?>','<?php echo $key; ?>')" name="invite"><i class="fas fa-spinner fa-spin loader-img loader-<?php echo $key; ?>"></i><i class="fas fa-user-plus userplus-<?php echo $key; ?>"></i></button>
                                                    </div>
                                                    <div class="snackbar btn-blue msg-<?php echo $key; ?>">Invite sent</div>
                                                </div>    
                                            </div>
...
<script>
    ...
    function sendInvitation(email,key){
        // console.log(email);
        // console.log(key);
        
        $('.loader-'+key).addClass('show');
        $('.userplus-'+key).hide();
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('invite')?>",
            data: { gemail: email, gkey : key, ajaxSubmit:true} ,
            success: function(response)
            {
                $('.loader-'+key).removeClass('show');
                $('.userplus-'+key).show();
                ...
                if (jsonData.success == "1")
                {
                    ...
                    //console.log('Invitation send Successfully to '+email+'!'+);
                    $('.msg-'+key).addClass('show');

                    setTimeout(function(){ 
                        $('.msg-'+key).removeClass('show');
                    }, 3000);

                    $('.invite-'+jsonData.key).html('<div class="btn invite-send post-post-button btn-blue"><i class="fa fa-paper-plane" aria-hidden="true"></i></div>');
                }
                else
                {
