<?php
  ...
  class Data_model extends CI_Model{
   ...
   public function display_media_by_id($id){
    ...
    public function update_user_profile($data, $email){
        $this->db->where('email', $email);
    ...
    public function update_user_interest($email, $interest){
        $query = $this->db->query("UPDATE users SET interest = '$interest' WHERE email = '$email' ");
        return $query;
    }
    ...
    public function isVideo($file='') {
        if(!empty($file)) {
            ...
            if(strtolower($fileInfo['extension'])=='mp4') return true;
        }
        return false;
    }
